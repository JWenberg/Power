...
public:
	...
	UAblAbilityTask* GetCurrentlySelectedAbilityTask() const;

	/* Toggles a run-time dependency to the provide Task. */
	void ToggleDependencyOnTask(UAblAbilityTask* Task, const UAblAbilityTask* DependsOn);
	...
	void SetCurrentTask(UAblAbilityTask* Task);
...
protected:
	...
	TWeakObjectPtr<UAblAbilityTask> m_CurrentTask;
