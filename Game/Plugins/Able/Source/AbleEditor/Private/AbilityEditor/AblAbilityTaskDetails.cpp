...
TSharedRef<IDetailCustomization> FAblAbilityTaskDetails::MakeInstance()
{
	return MakeShareable(new FAblAbilityTaskDetails());
}
...
FText FAblAbilityTaskDetails::GetTaskRealmText(IDetailLayoutBuilder* DetailBuilder) const
{
	if (UAblAbilityTask* Task = GetCurrentlyViewedTask(*DetailBuilder))
	{
		...
		{
		case EAblAbilityTaskRealm::Client: return LOCTEXT("ClientRealm", "Client");
		case EAblAbilityTaskRealm::ClientAndServer: return LOCTEXT("ClientAndServerRealm", "Client And Server");
		case EAblAbilityTaskRealm::Server: return LOCTEXT("ServerRealm", "Server");
