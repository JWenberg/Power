...
{
	const float WidgetHeight = float(m_Font.Size) * 2.0f;
...
int32 SAblAbilityTimelineNode::OnPaint(const FPaintArgs& Args, const FGeometry& AllottedGeometry, const FSlateRect& MyClippingRect, FSlateWindowElementList& OutDrawElements, int32 LayerId, const FWidgetStyle& InWidgetStyle, bool bParentEnabled) const
{
	...
	const float Size = ParentWidgetSize.X * TaskSizeRatio;
	float Offset = ParentWidgetSize.X * (GetNodeStartTime() / Ability->GetLength());
	...
	const FVector2D TextNodeSize(TextSize, ParentWidgetSize.Y);


	if (Offset >= ParentWidgetSize.X)
	{
		// We don't want to display off screen.
		Offset -= (Offset + Size) - ParentWidgetSize.X;
	}
	...
	if (TextTranslation.X + TextSize > ParentWidgetSize.X)
	{
		const float Overflow = (TextTranslation.X + TextSize) - ParentWidgetSize.X;
	...
	FSlateDrawElement::MakeBox(OutDrawElements,
		CurrentLayer++,
		AllottedGeometry.ToPaintGeometry(TextNodeSize, TextBackgroundOffset),
		m_TextBackgroundBrush);

	// Show our dependency icon.
	if (IsDependency)
	{
		const FVector2D IconOffset(TextBackgroundTranslation + FVector2D(2.0f, (ParentWidgetSize.Y - DependencyIconSize.Y) / 2));
	...
	FSlateDrawElement::MakeText(OutDrawElements,
		CurrentLayer++,
		AllottedGeometry.ToPaintGeometry(TextNodeSize, TextOffset),
		GetNodeText(),
		m_Font);
...
EAbilityTaskDragType SAblAbilityTimelineNode::DetermineDragType(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent) const
{
	...
	float Size = ParentWidgetSize.X * TaskSizeRatio;
	float Offset = ParentWidgetSize.X * (Task->GetStartTime() / Ability->GetLength());
...
void SAblAbilityTimelineTrack::Construct(const FArguments& InArgs)
{
	...
	m_TrackBrush = FAbleStyle::GetBrush("Able.AbilityEditor.Timeline");
}
