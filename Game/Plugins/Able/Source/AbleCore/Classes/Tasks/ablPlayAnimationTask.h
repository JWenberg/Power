...
protected:
	...
	void PlayAnimation(const UAnimationAsset* AnimationAsset, AActor& TargetActor, UAblPlayAnimationTaskScratchPad& ScratchPad, USkeletalMeshComponent& SkeletalMeshComponent, float PlayRate) const;
	...
	*/
	...
	/* If true, OnGetAnimationAsset will be called on the Ability and it will use the returned Animation Asset for this task. */
	UPROPERTY(EditAnywhere, Category = "Animation|Dynamic", meta = (DisplayName = "Use Dynamic Animation Asset"))
	bool m_UseDynamicAnimationAsset;

	/* A String identifier you can use to identify this specific task in the ability blueprint, when OnGetAnimationAsset is called. */
