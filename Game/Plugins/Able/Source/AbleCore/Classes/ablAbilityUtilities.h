...
public:
	...
	FAsyncAbilityInstanceUpdaterTask(UAblAbilityInstance* InAbilityInstance, float InCurrentTime, float InDeltaTime)
...
private:
	...
	TWeakObjectPtr<UAblAbilityInstance> m_AbilityInstance;
...
struct FAblFindAbilityInstanceByHash
{
	...
	bool operator()(const TWeakObjectPtr<UAblAbilityInstance>& A) const
	{
		check(A.IsValid());
...
struct FAbleLogHelper
{
	...
	static const FString GetResultEnumAsString(EAblAbilityStartResult Result);
	static const FString GetTaskResultEnumAsString(EAblAbilityTaskResult Result);
	static const FString GetTargetTargetEnumAsString(EAblAbilityTargetType Result);
	static const FString GetCollisionResponseEnumAsString(ECollisionResponse Response);
	static const FString GetCollisionChannelEnumAsString(ECollisionChannel Channel);
};