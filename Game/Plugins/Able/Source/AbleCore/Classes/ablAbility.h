...
public:
	...
	/* Returns whether or not our Cooldown is cached. */
	...
	/* (C++) Called when THIS Ability is attempting to interrupt another ability.
	...
	*/
	...
	/* (C++) Called by the Custom Filter for each actor that is a candidate to be acted upon by this Ability.
	...
	*/
	...
	/* (C++) Called when determining the cooldown for an Ability
	...
	*/
	virtual float CalculateCooldown(const UAblAbilityContext* Context) const { return GetBaseCooldown(); }
	...
	*/
	...
	/* (C++) Called when grabbing the Max stack count for this Ability (if it's a passive).
	...
	*/
	virtual int32 GetMaxStacks(const UAblAbilityContext* Context) const { return m_MaxStacks; }
	...
	*/
	...
	/* (C++) Called when checking if an Ability is valid to be started. You can use this event to execute
	...
	*/
	...
	/* (C++) Called by the Custom Targeting check. Any actors added to the FoundTargets value will be copied into the Ability Context.
	*/
	...
	*/
	...
	/* (C++) Called when checking if the looping segment of an Ability is valid to be started.
	...
	*/
	...
	/* (C++) Called by the Custom Channeling Condition.
	* @return - True if the channeling should be allowed to continue, false if not.
	*/
	virtual bool CheckCustomChannelConditional(const UAblAbilityContext* Context, const FName& EventName) const { return true; }
	...
	*/
	...
	bool CheckCustomChannelConditionalBP(const UAblAbilityContext* Context, const FName& EventName) const;

	/* (C++) Called when the Damage event needs to calculate a final damage value for the provided Actor.
	...
	*  NOTE: This method should always be thread safe, so only read data from actors - never write/change values of an actor. */
	...
	/* (C++) Called by the Custom Branch Conditional.
	...
	*/
	...
	/* (C++) Called when the Ability has passed all prerequisites (if any) and has actually started execution. */
	...
	UFUNCTION(BlueprintNativeEvent, Category = "Able|Ability", DisplayName="On Ability Start")
	void OnAbilityStartBP(const UAblAbilityContext* Context) const;

	/* (C++) Called when Ability has successfully executed all Ability Tasks. This is NOT called if the Ability is interrupted/branched.
	...
	*/
	 ...
	 * Use OnAbilityInerrupted/OnAbilityBranch to catch those cases.
	*/
	UFUNCTION(BlueprintNativeEvent, Category = "Able|Ability", DisplayName = "On Ability End")
	void OnAbilityEndBP(const UAblAbilityContext* Context) const;


	/* (C++) Called when an Ability is interrupted. */
	virtual void OnAbilityInterrupt(const UAblAbilityContext* Context) const { };

	/* Called when an Ability is interrupted. */
	UFUNCTION(BlueprintNativeEvent, Category = "Able|Ability", DisplayName = "On Ability Interrupt")
	void OnAbilityInterruptBP(const UAblAbilityContext* Context) const;

	/* (C++) Called when an Ability is being branched to another Ability. */
	...
	UFUNCTION(BlueprintNativeEvent, Category = "Able|Ability", DisplayName = "On Ability Branch")
	void OnAbilityBranchBP(const UAblAbilityContext* Context) const;

	/* (C++) Generic event called when a Task does a Collision query/Sweep. You can use EventName to identify which Task called this event. */
	virtual void OnCollisionEvent(const UAblAbilityContext* Context, const FName& EventName, const TArray<struct FAblQueryResult>& HitEntities) const { };
	...
	UFUNCTION(BlueprintNativeEvent, Category = "Able|Ability", DisplayName = "On Collision Event")
	void OnCollisionEventBP(const UAblAbilityContext* Context, const FName& EventName, const TArray<struct FAblQueryResult>& HitEntities) const;

	/* (C++) Generic event called when a Task does a Raycast query. You can use EventName to identify which Task called this event. */
	virtual void OnRaycastEvent(const UAblAbilityContext* Context, const FName& EventName, const TArray<FHitResult>& HitResults) const { };

	/* Generic event called when a Task does a Raycast query. You can use EventName to identify which Task called this event. */
	UFUNCTION(BlueprintNativeEvent, Category = "Able|Ability", DisplayName = "On Raycast Event")
	void OnRaycastEventBP(const UAblAbilityContext* Context, const FName& EventName, const TArray<FHitResult>& HitResults) const;


	/* (C++) Generic event called when by the Custom event Task. */
	...
	UFUNCTION(BlueprintNativeEvent, Category = "Able|Ability", DisplayName = "On Custom Event")
	void OnCustomEventBP(const UAblAbilityContext* Context, const FName& EventName) const;

	/* (C++) Generic event called by the Spawn Actor Task. You can use EventName to identify which Task called this event.*/
	virtual void OnSpawnedActorEvent(const UAblAbilityContext* Context, const FName& EventName, AActor* SpawnedActor) const { };

	/* Generic event called by the Spawn Actor Task. You can use EventName to identify which Task called this event.*/
	UFUNCTION(BlueprintNativeEvent, Category = "Able|Ability", DisplayName = "On Spawned Actor")
	void OnSpawnedActorEventBP(const UAblAbilityContext* Context, const FName& EventName, AActor* SpawnedActor) const;

	/* (C++) Called by the Branch Task if the dynamic branch ability option is enabled on the Task itself.
	* @return - The Ability to use as the Branching Ability.
	*/
	virtual UAblAbility* OnGetBranchAbility(const UAblAbilityContext* Context, const FName& EventName) const { return nullptr; }
	...
	*/
	...
	UAblAbility* OnGetBranchAbilityBP(const UAblAbilityContext* Context, const FName& EventName) const;

	/* (C++) Called when determining the play rate for an Ability
	...
	*/
	virtual float GetPlayRate(const UAblAbilityContext* Context) const { return GetBasePlayRate(); }
	...
	*/
	...
	float GetPlayRateBP(const UAblAbilityContext* Context) const;

	/* (C++) Called by the Cancel Ability Task to do any custom logic to determine whether or not to cancel the passed in Ability on the Task Target.
	...
	* @return - True to cancel the Ability, false to ignore it.
	*/
	UFUNCTION(BlueprintNativeEvent, Category = "Able|Ability", DisplayName = "Should Cancel Ability")
	bool ShouldCancelAbilityBP(const UAblAbilityContext* Context, const UAblAbility* Ability, const FName& EventName) const;

	/* (C++) Called when using a dynamic animation asset for Play Animation Tasks.
	* @return - The Animation Asset to play, must be a Sequence or Montage.
	*/
	virtual UAnimationAsset* OnGetAnimationAsset(const UAblAbilityContext* Context, const FName& EventName) const { return nullptr; }

	/* Called when using a dynamic animation asset for Play Animation Tasks.
	* @return - The Animation Asset to play, must be a Sequence or Montage.
	*/
	UFUNCTION(BlueprintNativeEvent, Category = "Able|Ability", DisplayName = "On Get Animation Asset")
	UAnimationAsset* OnGetAnimationAssetBP(const UAblAbilityContext* Context, const FName& EventName) const;
