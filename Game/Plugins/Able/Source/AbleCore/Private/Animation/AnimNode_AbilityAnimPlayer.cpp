...
void FAnimNode_AbilityAnimPlayer::Evaluate_AnyThread(FPoseContext& Output)
{
	...
	if (!m_AnimationQueue.Num())
	{
		...
		{
			m_CachedOutputSequence->GetAnimationPose(Output.Pose, Output.Curve, FAnimExtractContext(m_CachedOutputTime, Output.AnimInstanceProxy->ShouldExtractRootMotion()));
		}
		else
		{
	...
	switch (m_BlendType)
	{
		...
		{
			FAbilityAnimEntry& CurrentEntry = m_AnimationQueue[0];
			CurrentEntry.AnimationSequence->GetAnimationPose(Output.Pose, Output.Curve, FAnimExtractContext(CurrentEntry.TimeAccumulator, Output.AnimInstanceProxy->ShouldExtractRootMotion()));
		}
		break;
		case EvaluateBlendType::Multi:
		case EvaluateBlendType::SingleBlendOut:
		{
			...
			//else
			//{
			//	AlphaValue += CurrentEntry.BlendOut.GetAlpha();
			//}

			Weights[0] = 1.0f - AlphaValue;
			Weights[1] = AlphaValue;

			CurrentEntry.AnimationSequence->GetAnimationPose(Poses[0], Curves[0], FAnimExtractContext(CurrentEntry.TimeAccumulator, Proxy->ShouldExtractRootMotion()));
			if (NextEntry)
			{
				NextEntry->AnimationSequence->GetAnimationPose(Poses[1], Curves[1], FAnimExtractContext(NextEntry->TimeAccumulator, Proxy->ShouldExtractRootMotion()));
			}
			//else if (Weights[1] > SMALL_NUMBER)
			//{
			//	Poses[1] = Output.Pose;
			//	Curves[1] = Output.Curve;
			//}
