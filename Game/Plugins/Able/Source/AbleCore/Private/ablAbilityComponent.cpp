	...
	CooldownTime(1.0f)
{
	Ability = &InAbility;
	Context = &InContext;
	CooldownTime = Ability->CalculateCooldownBP(Context);
}
...
void UAblAbilityComponent::TickComponent(float DeltaTime, enum ELevelTick TickType, FActorComponentTickFunction *ThisTickFunction)
{
	...
	// Update Passives
	UAblAbilityInstance* Passive = nullptr;
...
EAblAbilityStartResult UAblAbilityComponent::CanActivatePassiveAbility(UAblAbilityContext* Context) const
{
	...
	if (const UAblAbility* Ability = Context->GetAbility())
	{
		...
		if (CurrentStackCount == 0 || CurrentStackCount < Ability->GetMaxStacksBP(Context))
		{
			return Ability->CanAbilityExecute(*Context);
		}
		else
...
{
	if (Ability.CalculateCooldownBP(&Context) > 0.0f)
	{
...
void UAblAbilityComponent::HandlePendingCancels()
{
	for (FAblPendingCancelContext& CancelContext : m_PendingCancels)
	{
		...
		if (m_ActiveAbilityInstance)
		{
			if (m_ActiveAbilityInstance->GetAbilityNameHash() == CancelContext.GetNameHash())
			{
			...
			{
				UAblAbilityInstance** foundInstance = m_PassiveAbilityInstances.FindByPredicate(FAblFindAbilityInstanceByHash(CancelContext.GetNameHash()));
				if (foundInstance && *foundInstance)
				{
					(*foundInstance)->FinishAbility();
					m_PassiveAbilityInstances.Remove(*foundInstance);
				}
			}
		}
	}
