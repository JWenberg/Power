...
bool UAblAbilityInstance::IsDone() const
{
	...
	{
		const uint32 MaxIterations = m_Ability->GetLoopMaxIterationsBP();
		// Zero = infinite.
		return MaxIterations != 0 && m_CurrentIteration >= MaxIterations;
	}
	else if (GetCurrentTime() > m_Ability->GetLength())
	{
...
{
	return GetAbility().GetPlayRateBP(m_Context);
}
...
void UAblAbilityInstance::SetCurrentIteration(uint32 NewIteration)
{
	m_CurrentIteration = NewIteration;
}
